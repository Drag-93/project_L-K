...
const AdminReserveOrdersModal = ({ item, onClose, onRefresh }) => {
  ...
  return (
    <div className="adminModal" onClick={onClose}>
      <div className="adminModal-con" onClick={(e) => e.stopPropagation()}>
        <div className="adminModal-title">예약 상세 정보</div>
        <span className="adminModal-close" onClick={onClose}>
          X
        </span>
        <div className="modal-body">
          ...
          <div className="info-group2">
            <b
              className={`reservstate ${state === "예약완료" ? "done" : "wait"}`}
            >
              {state}
            </b>
          ...
          <div className="info-group">
            ...
            <ul>
              <li className="info-row">
                <span>아이디(이메일):</span>{" "}
                <strong>{customer?.userEmail}</strong>
              </li>
              <li className="info-row">
        ...
        <div className="adminModal-footer">
          <div className="adminModal-footer-con">
            {item.state === "예약완료" ? (
              ...
              <>
                <button onClick={reserveOkFn} className="editBtn">
                  예약확정
                </button>{" "}
              </>
            )}
            <button onClick={reserveCancelFn} className="deleteBtn">
              예약취소
            </button>
