      ...
      try {
        ...
        const myReserves = resRes.data
          ...
          .map((order) => ({
            ...order,
            type: "reserve",
            dateForSort: order.reserveDate,
          }));

        // 데이터 합치기 및 최신순 정렬
        const combined = [...myProds, ...myReserves].sort(
          (a, b) => new Date(b.dateForSort) - new Date(a.dateForSort),
        );
    ...
    <div className="order_wrap">
      <div className="paylist_tab">
        <ul className="tab_menu">
          ...
          <li
            className={filter === "reserve" ? "active" : ""}
      ...
      <div className="list_search_wrap">
        <span className="list_search_length">
          결제내역 <b>{filteredList.length}</b>건
        </span>
            ...
            {pagedList.map((order) => (
              <div key={order.id} className="paylist_card product">
                <div className="paylist_header">
                  {order.type === "product" ? (
                    <span className="paylist_date">
                      {order.productUserDate}
                    </span>
                  ) : (
                    <span className="paylist_date">
                      {order.reserveUserDate}
                    </span>
                  )}
                </div>
                    ...
                    return (
                      <div key={idx} className="paylist_row">
                        ...
                        <div className="paylist_info">
                          ...
                          {item.state && (
                            <small
                              className={`paylist_state_badge ${item.state === "예약완료" || item.state === "배송완료" ? "ok" : ""}`}
                            >
                              {item.state}
                            </small>
                          )}
                ...
                <div className="paylist_footer">
                  ...
                  <span className="paylist_total_price">
