...
const ProdList = () => {
  ...
  // const firstPage = (Math.ceil(page / btnRange) - 1) * btnRange + 1;
  ...
  return (
    <div className="inner">
      <div className="sell_list_wrap">
        <img
          src={`/images/banner_${category || "all2"}.png`}
          className="sell_banner"
        />
        <div className="aside_wrap">
          <ul>
            <li>
              <NavLink to={`/product/list`} end>
                전체
              </NavLink>
            </li>
            <li>
              <NavLink to={`/product/list/hydro`}>보습</NavLink>
            </li>
            <li>
              <NavLink to={`/product/list/trouble`}>트러블</NavLink>
            </li>
            <li>
              <NavLink to={`/product/list/white`}>미백</NavLink>
            </li>
            <li>
              <NavLink to={`/product/list/antiage`}>안티에이징</NavLink>
            </li>
            <li>
              <NavLink to={`/product/list/uv`}>UV</NavLink>
            </li>
          </ul>
        </div>
        <div className="list_search_wrap">
          <span className="list_search_length">
            상품 <b>{list.length}</b>개
          </span>
                  ...
                  <Link to={`/product/detail/${el.category}/${el.id}`}>
                    <div className="top">
                      <img
                        src={
                          el.img
                            ? `/images/${el.category}/${el.img}`
                            : `/images/all_none.png`
                        }
                        alt={el.name}
                        onError={(e) => {
                          e.target.src = "/images/all_none.png";
                        }}
                      />
                    </div>
